...
## 2026-07-31 무료 플랜 남용 구멍 차단 (문서 수 → 문서+사건 이중 판정)

**구멍**: `requireUsageQuota`가 이번 달 **문서 수**만 셌다. 그런데 AI 분석은 문서보다 먼저
일어나므로, 문서를 만들지 않고 분석만 반복하면 카운트가 오르지 않아 무제한이었다.
실측 원가(Sonnet 5, 프로모션가 기준): 분석 1회 ≈ 9만 토큰 ≈ 700원 → **100회면 약 7만원.**

**차단**: 분석이 끝나면 사건이 자동 생성되므로 **사건 수 = 분석 횟수**다.
`documents >= 3 || cases >= 5`로 이중 판정. `countMonthly(env, uid, collection)`로 일반화하고
캐시 키를 `uid:컬렉션`으로 분리했다.

**화면-서버 기준 일치**: usePlanLimits가 `recordings`를 세고 있어 서버(cases)와 어긋났다.
녹음 문서는 파일 업로드 때만 생기므로(메모만 입력하면 안 생김) 지표로도 부정확했다.
→ 화면도 `cases`를 세도록 바꾸고 라벨을 "녹음" → "사건 분석"으로. 요금제 문구도 전부 정렬.

**원가 참고(무료 1인 월 최대)**: 사건 3건 기준 약 47만 토큰 ≈ 3,000원(프로모션) / 4,450원(정가).
Pro 89,000원이므로 무료 20명당 1명만 전환돼도 남는 구조. STT 비용은 별도.

## 2026-07-31 배경 영상 교체(v3) — "움직이지 않는다"의 진짜 원인

측정해보니 배경은 fixed로 고정돼 있었고 재생도 되고 있었다. 문제는 **영상 자체에 움직임이 없었던 것**.
프롬프트에 `static camera, almost no movement`를 넣은 게 원인 — 루프 이음새를 숨기려던 선택이
"정지 사진"으로 읽히게 만들었다.

정량 측정(0.5초 간격 프레임 픽셀 차이, 0=동일):
- 구 영상(kling, static 지정): **2.47**
- 신 영상(seedance1_5, "clearly moving mist" 지정): **53.09** — 21배
- 라이브 화면 1초 간격 변화: 1.25 → **22** (17배)

교훈: **배경 영상 프롬프트에 "움직임 없음"을 넣지 말 것.** 이음새는 왕복 루프로 해결하면 된다.
모델도 교체 — `seedance1_5` 720p 4초가 **2.4크레딧**으로 kling3_0_turbo(7.5)보다 훨씬 싸다.
CSS 줌(heroDrift)은 제거 — 영상이 충분히 움직이면 겹칠 필요가 없고 산만하다.
파일명은 v3로 올려 캐시 우회(같은 이름 덮어쓰기 금지 규칙 유지).
