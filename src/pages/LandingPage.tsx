...
function AnimatedCounter({
  ...
  // SSR 프리렌더 시점에는 애니메이션이 실행되지 않으므로 최종값을 바로 노출(검색·AI 크롤러용)
...
function useTypewriter(words: string[], typingSpeed = 100, pauseTime = 2000) {
  // SSR 프리렌더 시점에는 타이핑 애니메이션이 실행되지 않으므로 첫 단어를 완성된 상태로 노출
  const [text, setText] = useState(() => (typeof window === "undefined" ? words[0] : ""));
...
export default function LandingPage() {
  const typewriterText = useTypewriter(
    ["판례 검색", "문서 작성", "계약서 생성", "성공보수 관리", "소송비용 계산", "의뢰인 케어"],
    80,
    1800
  );
        ...
        <div className="max-w-5xl mx-auto">
          <FadeIn>
            <div className="text-center mb-16">
              ...
              <p className="text-[#414846] text-lg">
                사무장 월급보다 저렴한 비용으로, 변호사 업무 전체를 자동화하세요.
              </p>
            </div>
          </FadeIn>
